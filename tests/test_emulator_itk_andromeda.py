...
from comet.emulator.itk.andromeda import AndromedaEmulator


@pytest.fixture
def emulator():
    return AndromedaEmulator(Context())


def test_basic(emulator):
    assert emulator("identify") == "Andromeda 0 0 0 0"
    assert emulator("getversion") == "1.0"
    assert emulator("version") == "1.0"
    assert emulator("getmacadr") == "00:00:00:00:00:00"
    assert emulator("getserialno") == "00770031"
