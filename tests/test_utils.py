...
def test_to_unit():
    assert utils.to_unit(42, "V") == 42.0
    assert utils.to_unit(42, "mV") == 42.0
    assert utils.to_unit("42V", "V") == 42.0
    assert utils.to_unit("42mV", "V") == 0.042
...
def test_combine_matrix():
    ...
    assert utils.combine_matrix("ab", "12", "XY") == [
        "a1X",
        "a1Y",
        "a2X",
        "a2Y",
    ...
    assert utils.combine_matrix("12", "AB", ["08", "16"]) == [
        "1A08",
        "1A16",
        "1B08",
        "1B16",
...
def test_inverse_square():
    ...
    assert utils.inverse_square(2) == 0.25
    assert utils.inverse_square(8) == 0.015625
