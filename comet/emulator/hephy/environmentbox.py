...
class EnvironmentBoxHandler(IEC60488Handler):
    ...
    def query_get_pc_data(self):
        return ','.join(map(format, type(self).pc_data()))

    @message(r'SET:LASER_SENSOR (ON|OFF)')
    def query_set_laser_sensor(self, value):
        type(self).laser_sensor = value == 'ON'
        return 'OK'

    @message(r'SET:BOX_LIGHT (ON|OFF)')
    def query_set_box_light(self, value):
        type(self).box_light = value == 'ON'
        return 'OK'

    @message(r'GET:MICROSCOPE_LIGHT ?')
    def query_get_microscope_light(self):
        return int(type(self).microscope_light)

    @message(r'SET:MICROSCOPE_LIGHT (ON|OFF)')
    def query_set_microscope_light(self, value):
        type(self).microscope_light = value == 'ON'
        return 'OK'

    @message(r'GET:MICROSCOPE_CAM ?')
    def query_get_microscope_camera(self):
        return int(type(self).microscope_camera)

    @message(r'SET:MICROSCOPE_CAM (ON|OFF)')
    def query_set_microscope_camera(self, value):
        type(self).microscope_camera = value == 'ON'
        return 'OK'

    @message(r'GET:MICROSCOPE_CTRL ?')
    def query_get_microscope_control(self):
        return int(type(self).microscope_control)

    @message(r'SET:MICROSCOPE_CTRL (ON|OFF)')
    def query_set_microscope_control(self, value):
        type(self).microscope_control = value == 'ON'
        return 'OK'

    @message(r'GET:PROBCARD_LIGHT ?')
    def query_get_probecard_light(self):
